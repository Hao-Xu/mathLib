...
struct Unsymmeig {
    int n;
    r2Tensor<double> a,zz;
    r1Tensor< complex<double> > wri;
    r1Tensor<double> scale;
    r1Tensor<int> perm;
    bool yesvecs,hessen;

    Unsymmeig(const r2Tensor<double> &aa, bool yesvec=true, bool hessenb=false) :
        n(aa.dim1()), a(aa), zz(n,n,0.0), wri(n), scale(n,1.0), perm(n),
        ...
        do {
            ...
            if (l == nn) {
                ...
                if (l == nn-1) {
                    ...
                    if (its == 30) throwout("Too many iterations in hqr");
                    if (its == 10 || its == 20) {
                        t += x;
        ...
        for (nn=n-1;nn>=0;nn--) {
            ...
            if (q == 0.0) {
                ...
                if (abs(a[nn][na]) > abs(a[na][nn])) {
                    ...
                    complex<double> temp=complex<double>(0.0,-a[na][nn])/complex<double>(a[na][na]-p,q);
                    a[na][na]=real(temp);
                    a[na][nn]=imag(temp);
                }
                a[nn][na]=0.0;
                ...
                for (i=nn-2;i>=0;i--) {
                    ...
                    if (imag(wri[i]) < 0.0) {
                        ...
                        if (imag(wri[i]) == 0.0) {
                            ...
                            complex<double> temp=complex<double>(x*r-z*ra+q*sa,x*s-z*sa-q*ra)/
                            ...
                            if (abs(x) > abs(z)+abs(q)) {
                                ...
                                complex<double> temp=complex<double>(-r-y*a[i][na],-s-y*a[i][nn])/
                                    complex<double>(z,q);
                                a[i+1][na]=real(temp);
                                a[i+1][nn]=imag(temp);
                            }
